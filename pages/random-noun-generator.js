...
const Home = () => {
  ...
  useEffect(() => {
    handleGenerateRandomNoun()
  }, [])


  const handleGenerateRandomNoun = () => {
    setNoun(faker.word.noun().toLowerCase())
  }

  return (
    <div>
      <SEOMeta title="Random Noun Generator" description="Random Noun Generator" />
      <main className="m-auto flex max-w-5xl flex-col px-4">
        <Nav />
        <div className="flex flex-col gap-8 pt-2">
          <h1 className="text-3xl">Random Noun Generator</h1>
          <div className="flex flex-col gap-4 items-center border h-80 sm:h-none justify-center sm:w-1/2">
            <span onClick={handleCopyToClipboard} className="flex gap-2 justify-center items-center cursor-pointer">
              <span className="text-4xl font-extrabold">{noun}</span>
            ...
            <button onClick={handleGenerateRandomNoun} className="p-2 flex gap-3 px-3 mt-3 items-center bg-blue-500 text-white rounded-md w-fit ">
